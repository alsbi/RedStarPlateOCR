...
def test_full_mask_standard_returns_seq_len():
    ...
    comp = AdaptiveCompression(canvas_height=80, canvas_width=256, stride=4)
    # feat_h=20, feat_w=64
    mask = torch.ones(2, 1, 20, 64)
    result = comp.compute_input_lengths(mask, ["standard", "standard"])
    assert result.shape == (2,)
    assert result[0].item() == 64 + 4  # +4 safety margin
    assert result[1].item() == 64 + 4
...
def test_square_type_top_w_plus_bot_w():
    ...
    # feat_w(64) + bot_present(6) + safety_margin(4) = 74
    assert result[0].item() == 74
...
def test_clamp_min_2():
    ...
    clamped = result.clamp(min=2, max=64)
...
def test_mixed_batch():
    ...
    assert result[1].item() == 83  # feat_w(64) + bot_present(15) + margin(4)
...
def test_square_partial_content_mask():
    ...
    comp = AdaptiveCompression(canvas_height=80, canvas_width=256, stride=4)
    mask = torch.zeros(1, 1, 20, 64)
    # Top half: all 64 cols filled
    mask[0, 0, :10, :] = 1.0
    # Bottom half: only 10 cols filled
    mask[0, 0, 10:, :10] = 1.0
    result = comp.compute_input_lengths(mask, ["square"])
    # feat_w(64) + bot_present(10) + safety_margin(4) = 78
    assert result[0].item() == 78
